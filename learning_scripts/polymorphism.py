...
class Company(Client):  #child class, company as sub class of client
    def __init__(self, name, email, rep_number):
...
class Individual(Client): #child class
    def __init__(self, name, email, recent_purchases):
...
jason = Individual("Jason Mraz", "jmz@singer.com", False)
exon = Company("Exon Mobile", "em@emobile.com", "719-650-4565")
jason.get_info()
jason.make_purchase()
jason.get_info()
exon.get_info()
